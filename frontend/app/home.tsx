    ...
    const cargarDatosYToken = async () => {
      try {
        ...
        const pushToken: string =
          (expoToken as any).data ??
          (expoToken as any).pushToken ??
          (expoToken as any);

        console.log("📱 Expo pushToken:", pushToken);

        await fetch("http://192.168.12.197:4000/api/notificaciones/token", {
          method: "POST",
          headers: {
  ...
  const avisos = useMemo(
    () =>
      reservas
        .map((r) => ({
          ...r,
          diff: daysDiffFromToday(r.fechaISO),
        }))
    ...
    <SafeAreaView style={s.safeArea}>
      <View style={s.container}>
        ...
        <View style={s.titleWrapper}>
          ...
          <View style={s.statsChip}>
            <Text style={s.statsText}>
              {tieneReservas
                ? `Tienes ${reservasVigentes.length} reserva${
                    reservasVigentes.length > 1 ? "s" : ""
                  }`
        ...
        <Modal
          ...
          transparent
          animationType="fade"
          onRequestClose={() => setOpenAvisos(false)}
        >
            ...
            <View style={s.modalCard}>
              ...
              {avisos.length === 0 ? (
                <Text style={s.modalEmpty}>
                  No tienes reservas en los próximos días.
                </Text>
              ) : (
                avisos.map((r) => (
                  <View key={r.id} style={s.modalRow}>
                    <Text style={s.modalDate}>
                      {toDDMMYYYY(r.fechaISO)} • {r.hora}
                    </Text>
